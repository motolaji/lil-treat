...
```text
- This screen shows treats being successfully added to the users app after
they scan the vendor's QR code. For users that are not logged in, it shows
a section asking the user to install. 

- Extract the logic that determines if the user is logged in to a common file
...
had before but this is not an overlay, it is on the page itself. As such,
it is fine to have this be a new component separate from the overlay component.
But it might make sense to share styles if possible.

- For now, wire the "Claim a Treat" button to direct to this screen.
```
