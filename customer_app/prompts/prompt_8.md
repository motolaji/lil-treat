...
```text
- This screen is not a new screen - it is just an expanded form of the treats
on the home page.

- Note that there are cards with white borders and glow. By default these are
how all cards will appear unless their points are 80% or more (at which point they 
will be marked as yellow) or 100% or more (at which they will be marked as green).
Put enough cards on the treats page so we can test the expanded form

- The user can trigger the pane two expand up in a couple of ways:
  1. They try to scroll on their treats
  2. They tap on the white handle at the top of the pane or drag the handle up
  3. They tap on the search button
  
- If the user expanded the pane through 1) or 2) then the pane will expand and
the search icon will still remain in place (no search bar will be shown)

- If the user expanded the pane through 3) then the pane will expand and
the search icon will dissapear - replaced by the search bar. Note that the
search bar shown here is the same as the TextField already created just with the
colours inverted. As such, reuse that component.

- If the user pulls down to minify the treats section, the search bar should
dissappear and the search icon should show (if the search bar was present in the
expanded state)

- Similar to the other screens - ignore the help button at the bottom right
```
