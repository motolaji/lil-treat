...
```text
1. This screen is an overlay modal for the filter functionality for the filter
button used on multiple screens. Because it is a highly used component we
want to make it generic. We this component to take the sort options and
filter options as a json config. Define the type to be something similar to:

{
    "sortOptions": [
        {
            "sortOptionKey": "location",
            "sortOptionLabel": "Closest Vendor First", 
        },
        {
            "sortOptionKey": "treats",
            "sortOptionLabel": "Least Treats to Redeem Big Treat", 
        }
    ],
    "filterOptions": [
        {
            "filterOptionKey": "vendorCategory",
            "filterOptionLabel": "Filter by Vendor Category"
            "filterOptionValues": [
                {"filterOptionValueKey": "drinks", "filterOptionValueLabel": "Drink"},
        ...
        },
        {
            ...
            "filterOptionValues": [
                {"filterOptionValueKey": "1km", "filterOptionValueLabel": "< 1km away"},
                {"filterOptionValueKey": "5km", "filterOptionValueLabel": "< 5km away"},
            ]
        }
    ]
}

It should take a sortAndFilterFunction(), that accepts something similar to:

{
    "sortOption": "location" // or null (if nothing selected)
    "filterOptions": [
        "vendorCategory": "drinks",
        "vendorDistance": "1km"
    ] // or empty array if nothing selected
}


2. In the current screenshot there is only one filter value but we will
allow multiple dropdowns for each filter option provided by the calling
function

3. The overlay is showing a white overlay background but use a dark overlay
similar to the other overlay modals.

4. Wire up the new filter modal component to the screens that have the
