...
```text
- This is a modal overlay similar to the one on the receipts page. In fact,
we want to create a generic component that will be used by both the receipts
page and this page. The component should have a primary and secondary button
and accept functions to run when these buttons are clicked as well as labels
for those buttons. Both buttons will dismiss the modal when clicked. For now
use a dummy QR code for the redeem treat modal
```
