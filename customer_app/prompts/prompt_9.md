...
```text
- Note that this uses the same card style as the treats card. But has no
progress bar. You might want to have a generic version of the card as a
component that just has the core layout elements and display detail and have
the TreatsCardContent as a separate component. However, analyze to determine
if this is the best route to achieve reusability without overloading a single
component
```
