...
```text
- This screen shows the user's profile detail when they are logged in. Currently
when the user clicks on their profile icon on the sidebar we send them to the
login view. Change the behaviour as follows:

1. Firstly add a state to your mock user state object that captures how the
user logged in: email vs google sign in.

2. If the user is not logged in, and taps on their account icon in the sidebar,
send them to the login page as usual.

3. If the user is logged in and they logged in through email, send them to
this screen that you are about to implement.

4. There will be another screen where the user will get routed to if they
logged in via google. This is not implemented yet. For now if the user logged
in via google and they tap their account icon send them back to the home page.
```
