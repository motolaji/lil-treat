...
```text
- This screen is a subtab of the VendorScreen. Note that the design of the
tab selector itself has changed between when it was implemented and the
current mock. Update the tab selector design to be inline with this screen.

- The categories on the items listed are supposed to be scrollable horizontally.
The user may scroll it by swiping to the left or use the chevron. Add a few other
categories so I can test this scrollability.
```
