...
```text
home_page_pending_email_verification
```
...
```text
The only thing you need to implement from this page is the black pill at the top highlighting that the user
hasn't verified their email. Tapping on that should bring an overlay modal with content like "Please verify your 
email by clicking on verification link sent to ${userEmailAddress}. If you are yet to receive the email, we can
'resend' you the verification link. Where the resend is hyperlinked. There should be one primary button to 'Close'"

Ignore adding the drop-shadow to the pill
