...
```text
- It may not be obvious from the screen but this is a modal overlay that will 
pop-up when the user taps on a receipt to view the full receipt detail. Don't
implement as a separate screen. The modal should be dismissed if the user taps
outside the modal or taps on the close button
```
