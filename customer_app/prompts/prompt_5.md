...
```text
- This screen uses the input field similar to the treat jar page. As such extract
this input field to be its own component which can have an optional label and
optional placeholder. Validation errors will be displayed below the field if
applicable. Note that for the search use-case there won't be validation errors
so implement the component with that in mind.

- The current pair of search input field and filter used on the treat jar
page should also be extracted out to its own component as this pair will be
used across other screens.

- For now, wire the account icon + detail on the sidebar of the homepage to bring
us to this login screen. The homepage of the login screen should send us back
to the homepage.

- Don't fix the treat jar art to the bottom of the page (i.e. it should behave
differently from the treat jar page). It should just scroll with the rest of the
page content if the user's screen is small
```
