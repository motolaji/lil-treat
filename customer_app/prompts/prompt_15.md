...
```text
- The flow after QR code scanning is as follows:

1. If the user is not logged in, we will take them to the login prompt. If
they skip for now. We will check if the app is installed and send them to the
app install prompt (this screen). If they don't skip and go to the install
screen then there exists no path for them to be sent to the app install screen
as the back button from the login screen will take them home and this is fine.

2. If the user is logged in but the app is not installed then we will send
them directly to the prompt install screen. Modify the content in the prompt
so it doesn't show full steps but rather says something along the lines of "To
attempt installing the app, tap install now" and change the "Done" text to
"Install Now". Tapping on install now should send the user to the installation
screen which already has an automated install prompt (if available on the
device)

3. If the user is logged in and installed. We will show a third screen (we
will implement this later) that just shows the success content and a
button to View My Treats which sends them back to the home page.

- Extract the logic that determines if the user has installed to a common file
that can be reused as this will be checked in a couple of different places on
the app to influence slight behaviour variations.

- Add a flag for the mock user you have created to specify if the app is installed
for that user

- Extract the success content on both the login prompt and install prompt to
its own component as this should be kept in sync and the same content will
be used on the third screen (yet to be implemented)

- On this screen you will see what looks like the modal overlay that we
had before but this is not an overlay, it is on the page itself. This
on page modal was used on the install prompt as well. Extract it to its
own component that can be used by both this screen and the install prompt
screen.
```
