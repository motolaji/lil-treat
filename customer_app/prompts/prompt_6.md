...
```text
- This screen uses the same components from the sign up page. Leverage those
to build it out

- Wire up the "Sign Up" text on the login screen to bring the user to the
sign up screen. Similarly, wire up the "Log In" button on the Sign Up page to
send the user back to the "Log In" screen. If the user selects Login
from the sign up screen navigate back to the login (if that was the previous page)
instead of navigating forward to prevent an infinite navigation log.
Note that in some cases the user may go to the sign up page directly through
the URL. Only in that case, move them forward to the login screen.
```
