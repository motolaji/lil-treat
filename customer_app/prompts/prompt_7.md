...
```text
- This screen leverages a header that blends into the navbar because it has
the same colour. 

- It has a sub-tabs for flipping between redeeming big treats
collecting LilTreats. For now enable toggling between the two but keep the
Collect LilTreats empty when I switch to that sub-tab

- The card on this screen is a bit similar to the treatcard but it has enough
of a difference to be its own component. It does leverage the progress bar though.
Extract the progress bar (including the text above it the metric 100/100) to its
own component and share it in all cards that use this. Also, this page defines
darker green and yellow because it is on a white background. Also, I see a colors.ts
file was created but not used so far. Leverage this to define the shared colours - if
it is better to create a CSS copy of this file then do that show all style sheets
can reference a single source instead of defining the colours disparately
```
